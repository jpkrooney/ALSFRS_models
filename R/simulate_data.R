simulate_data_from_registry <- function(standardized_data,
    ...
    #Subsample to max_measurements_per_subject, but keep first and last
    ...
    bad_counts <- subsampled_data %>% group_by(subject_id) %>%
        summarise(n_first = sum(is_first), n_last = sum(is_last),
                  n_total = n(), .groups = "drop") %>%
        filter(n_first != 1 | n_last != 1 |
                   n_total < min_measurements_per_subject |
                   n_total > max_measurements_per_subject)
    ...
    patient_stats <- subsampled_data %>%
        ...
        summarise(
            ...
            criterium = if_else(choice_criterium == "slope", slope, alsfrs_end)
        ) %>%
        arrange(alsfrs_start, duration) %>%
        ...
        mutate(pick_better = rbinom(1, size = 1, prob = effect_prob) == 1,
               treatment_subject_id = if_else(pick_better,
                                              unique(subject_id[which.max(criterium)]),
                                              unique(subject_id[which.min(criterium)])),
