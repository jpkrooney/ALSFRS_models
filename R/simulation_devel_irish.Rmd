...
#any(!is.na(irldat$Q5a_Cutting_without_Gastrostomy) & !is.na(irldat$Q5b_Cutting_with_Gastrostomy))
...
#    rename_with(function(x) { question_col_names_translation[x]}, .cols = #all_of(question_col_names_irldat)) %>%
...
sim1 <- simulate_data_from_registry(irldat_standardized,
                                    max_duration = 36,
...
for(i in 1:length(eff_probs)) {
  sims[[i]] <- simulate_data_from_registry(irldat_standardized,
                                    max_duration = 36,
                                    min_measurements_per_subject = 2,
                                    max_measurements_per_subject = 8,
...
# fits_mv_probit <- brm_parallel(args_shared = list(formula = bf(irldat_formula, family = empty_cumulative()) + set_rescor(FALSE), stanvars = sv, adapt_delta = 0.95, init = 0.1), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE)


summaries_mv_probit <- brm_parallel(
      args_shared = list(formula = bf(irldat_formula, family = empty_cumulative()) +
                           set_rescor(FALSE), stanvars = sv, adapt_delta = 0.95, init = 0.1),
      args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE,
...
```
```{r}
fits_logit <- brm_parallel(args_shared = list(formula = bf(irldat_formula, family = cumulative()) + set_rescor(FALSE), adapt_delta = 0.95, init = 0.1), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE)
...
get_spline_coeffs <- function(x) {
  res <- ns(x, knots = 12, Boundary.knots = c(0, 12))
