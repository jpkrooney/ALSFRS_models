...
fit_rescor_bgoodri <- brm(bf(proact_formula, family = empty_cumulative()) + set_rescor(FALSE), data = data_for_fit, stanvars = sv, adapt_delta = 0.95, inits = 0.1, file = paste0(cache_dir, "/test_simul_bgoodri"), file_refit = "on_change")
...
for(i in 1:length(eff_probs)) {
  data_for_fit <- sims[[i]] %>%
    mutate(across(all_of(question_col_names), function(x) { x + 1 }))
  fits[[i]] <- brm(bf(proact_formula, family = empty_cumulative()) + set_rescor(FALSE), data = data_for_fit, stanvars = sv + sum_to_zero_stanvar, adapt_delta = 0.95, inits = 0.1, file = paste0(cache_dir, "/test_simul_bgoodri_", i), file_refit = "on_change")
