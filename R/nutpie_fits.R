...
stancode <- brms::make_stancode(bf(irldat_formula, family = empty_cumulative()) +
...
model <- nutpie_compile_model(code = stancode)
...
brmsfits <- lapply(1: length(eff_probs), function(i) {
        ...
        #
        rstanobj <- nutpieR_to_rstan_obj(nutfits[[i]], stanmodel)
        brmsfit <- emptybrms
        brmsfit$fit <- rstanobj
        brmsfit <- rename_pars(brmsfit)
        brmsfit}) #, .progress = TRUE, .options = furrr_options(seed = 123))

saveRDS(brmsfits, paste0(cache_dir, "/", "20260402_nutpiemvprobit_fits_test.RDS"))
