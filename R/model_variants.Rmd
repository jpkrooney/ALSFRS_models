...
source(here::here("R", "mv_probit_augmented.R"))
...
N_cat <- 5
N_row <- nrow(simple_test)


stanvars_mult_probit <- make_stanvars_ordinal_augmented(
  column_names = question_names, N_cat = 5, 
  threshold_prior_family = "student_t",
  threshold_prior_args = "3, 0, 5"
  )

f <- bf(mvbind(q1,q2,q3) ~ 1 + time_from_diagnosis + (1 | p | patient), family = empty_ordinal(N_cat = 3)) + set_rescor(FALSE)

stancode <- make_stancode(f, data = simple_test, stanvars = stanvars_mult_probit)
stancode
...
model_data <- make_standata(f, data = simple_test, stanvars = stanvars_mult_probit)
...
Code based on https://github.com/stan-dev/example-models/commit/d6f0282d64382b627dfddca6b7f9a551bda3f537 and advice on Stan Discourse by @CerulloE
