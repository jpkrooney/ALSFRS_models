...
source(here::here("R", "mv_probit_bgoodri.R"))
...
single_test <- generator_multivariate_probit_augmented(N_obs = 100, N_cat = 2, N_dim = 4)
...
saveRDS(sbc_res, file = paste0(cache_dir, "/sbc_mv_probit_bgoodri2cat.rds"))
...
```{r}
cmdstan_fit <- model_single$sample(data = single_test$observed, adapt_delta = 0.95)
fit <- rstan::read_stan_csv(cmdstan_fit$output_files())
