...
setup_cluster(max_server_workers = 18)
cores_per_fit <- 4
chains <- 4
iter <- 1000
warmup <- 1000
...
n_subjects_per_group <- 100
#eff_probs <- rep(c(0.5, 0.7), times = 2)
...
sv <- make_stanvars_mv_probit_bgoodri(questions_to_fit)
if(keep_fits) {
fits_mv_probit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = empty_cumulative()) + rescor, stanvars = sv, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_mv_probit <- purrr::map(fits_mv_probit, effect_from_sim_study)

} else {
summaries_mv_probit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = empty_cumulative()) + rescor , stanvars = sv, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
...
if(keep_fits) {
fits_mv_probit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = empty_cumulative())  + rescor, stanvars = sv, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_mv_probit_cor <- purrr::map(fits_mv_probit_cor, effect_from_sim_study)

} else {
summaries_mv_probit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = empty_cumulative()) + rescor, stanvars = sv, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
}
...
if(!skip_nocor) {
if(keep_fits) {
  fits_logit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_logit <- purrr::map(fits_logit, effect_from_sim_study)

} else {
    summaries_logit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
...
if(keep_fits) {
  fits_logit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_logit_cor <- purrr::map(fits_logit_cor, effect_from_sim_study)

} else {
    summaries_logit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = chains, iter = iter, warmup = warmup), args_per_fit = args_per_fit,
                               cores_per_fit = cores_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
