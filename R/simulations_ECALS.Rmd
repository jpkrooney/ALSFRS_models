...
options(mc.cores = parallelly::availableCores(), brms.backend = "cmdstanr")
...
```{r}
setup_cluster(max_server_workers = 40)
```
...
n_subjects_per_group <- 75
#eff_probs <- rep(c(0.5), times = 200)
eff_probs <- rep(c(0.5, 0.8), times = 100)
keep_fits <- FALSE

# Final scenario
# questions_to_fit <- question_col_names
# n_subjects_per_group <- 100
# eff_probs <- rep(c(0.5, 0.7), times = 100)
# #eff_probs <- rep(c(0.5), times = 100)
# keep_fits <- FALSE


skip_mv_probit <- FALSE
skip_correlations <- FALSE
skip_nocor <- TRUE


if(length(questions_to_fit) == 1) {
    rescor <- NULL
} else {
    rescor <- set_rescor(FALSE)
}
...
#    ") ~ 1 + s(alsfrs_dly_mnths, by = group) + (1 + alsfrs_dly_mnths | subject_id)"
    ") ~ 1 + dly_mnths_spline_1 * group + dly_mnths_spline_2 *
            group + (1 + dly_mnths_spline_1 + dly_mnths_spline_2 |
                        subject_id)"

))

proact_formula_cor <- as.formula(paste0(
    "mvbind(", paste0(questions_to_fit, collapse = ", "),
#    ") ~ 1 + s(alsfrs_dly_mnths, by = group) + (1 + alsfrs_dly_mnths | p | subject_id)"
    ") ~ 1 + dly_mnths_spline_1 * group + dly_mnths_spline_2 *
            group + (1 + dly_mnths_spline_1 + dly_mnths_spline_2 | p |
                        subject_id)"
))
...
prog_sim <- progressr::progressor(length(eff_probs))
for(i in 1:length(eff_probs)) {
  sims[[i]] <- simulate_data_from_registry(proact_standardized,
   ...
   sims[[i]] <- cbind(sims[[i]],
                           get_spline_coeffs(sims[[i]]$alsfrs_dly_mnths))
...
```{r}
if(!skip_mv_probit && !skip_nocor) {
sv <- make_stanvars_mv_probit_bgoodri(questions_to_fit)
if(keep_fits) {
fits_mv_probit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = empty_cumulative()) + rescor, stanvars = sv, adapt_delta = 0.95, init = 0.1), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_mv_probit <- purrr::map(fits_mv_probit, effect_from_sim_study)

} else {
summaries_mv_probit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = empty_cumulative()) + rescor , stanvars = sv, adapt_delta = 0.95, init = 0.1), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
}

all_effects_mv_probit <- purrr::map_df(1:length(eff_probs), 
                         ~ data.frame(id = .x, type = "mv_probit", eff_prob = eff_probs[.x], estimate = summaries_mv_probit[[.x]]$effect_samples))

all_diags_mv_probit <- purrr::map_dfr(1:length(eff_probs), 
                         ~ summaries_mv_probit[[.x]]$diags |> mutate(id = .x, type = "mv_probit", .before = 1))
}

```

## MV probit fits - correlations

```{r}
if(!skip_mv_probit & !skip_correlations) {
if(keep_fits) {
fits_mv_probit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = empty_cumulative())  + rescor, stanvars = sv, adapt_delta = 0.95, init = 0.1), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_mv_probit_cor <- purrr::map(fits_mv_probit_cor, effect_from_sim_study)

} else {
summaries_mv_probit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = empty_cumulative()) + rescor, stanvars = sv, adapt_delta = 0.95, init = 0.1), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
}

all_effects_mv_probit_cor <- purrr::map_df(1:length(eff_probs), 
                         ~ data.frame(id = .x, type = "mv_probit_cor", eff_prob = eff_probs[.x], estimate = summaries_mv_probit_cor[[.x]]$effect_samples))

all_diags_mv_probit_cor <- purrr::map_dfr(1:length(eff_probs), 
                         ~ summaries_mv_probit_cor[[.x]]$diags |> mutate(id = .x, type = "mv_probit_cor", .before = 1))
...
if(!skip_nocor) {
if(keep_fits) {
  fits_logit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = 2, iter = 3000, warmup = 1000), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_logit <- purrr::map(fits_logit, effect_from_sim_study)

} else {
    summaries_logit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = 2, iter = 3000, warmup = 1000), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
    
}

all_effects_logit <- purrr::map_df(1:length(eff_probs), 
                         ~ data.frame(id = .x, type = "logit", eff_prob = eff_probs[.x], estimate = summaries_logit[[.x]]$effect_samples))

all_diags_logit <- purrr::map_dfr(1:length(eff_probs), 
                         ~ summaries_logit[[.x]]$diags |> mutate(id = .x, type = "logit", .before = 1))
}
```
...
## Logit fits - correlations

```{r}
if(!skip_correlations) {
if(keep_fits) {
  fits_logit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = 2, iter = 3000, warmup = 1000), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE)

  summaries_logit_cor <- purrr::map(fits_logit_cor, effect_from_sim_study)

} else {
    summaries_logit_cor <- brm_parallel(args_shared = list(formula = bf(proact_formula_cor, family = cumulative()) + rescor, adapt_delta = 0.95, init = 0.1, chains = 2, iter = 3000, warmup = 1000), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE, cache_summaries = TRUE, summarise_fun = effect_from_sim_study,
                                    future.globals = list("effect_from_sim_study" = effect_from_sim_study))
    
}

all_effects_logit_cor <- purrr::map_df(1:length(eff_probs), 
                         ~ data.frame(id = .x, type = "logit_cor", eff_prob = eff_probs[.x], estimate = summaries_logit_cor[[.x]]$effect_samples))

all_diags_logit_cor <- purrr::map_dfr(1:length(eff_probs), 
                         ~ summaries_logit_cor[[.x]]$diags |> mutate(id = .x, type = "logit_cor", .before = 1))
}
```
...
progressr::with_progress({
progressor_lmer <- progressr::progressor(along = sims)
fits_lmer_splines <- lapply(sims, function(data_for_lmer) {
...
progressr::with_progress({
progressor_lmer <- progressr::progressor(along = sims)
fits_lmer_simple <- lapply(sims, function(data_for_lmer) {
  data_for_lmer$alsfrs_sum_subset <-  rowSums(data_for_lmer[,questions_to_fit])
  res <- lme4::lmer(alsfrs_sum_subset ~ 1 + alsfrs_dly_mnths*group+ (1 + alsfrs_dly_mnths |subject_id), data = data_for_lmer)
  progressor_lmer()
  res
})
})
...
```{r, fig.width=10}
summarise_brms <- function(all_effects, all_diags) {
  all_effects %>% filter |> 
     filter(id %in% which(all_diags$n_divergent <= 80 & all_diags$max_rhat <= 1.05))   |>      
...
if(!skip_nocor) {
    summaries_list[[length(summaries_list) + 1]] <- summarise_brms(all_effects_logit, all_diags_logit)
} 

if(!skip_correlations) {
    summaries_list[[length(summaries_list) + 1]] <- summarise_brms(all_effects_logit_cor, all_diags_logit_cor)
}


if(!skip_mv_probit) {
    if(!skip_nocor) {
      summaries_list[[length(summaries_list) + 1]] <- summarise_brms(all_effects_mv_probit, all_diags_mv_probit)
    }
    if(!skip_correlations) {
      summaries_list[[length(summaries_list) + 1]] <- summarise_brms(all_effects_mv_probit_cor, all_diags_mv_probit_cor)
    }
}

summaries_all <- do.call(rbind, summaries_list)

summaries_all |>
    mutate(detect = if_else(eff_prob ==0.5, sign(lower95)==sign(upper95), lower95 > 0),
...
summaries_all %>%
  ...
  ggplot(aes(x = factor(id), group = interaction(id, type), color = type)) + 
    ...
    facet_wrap(~eff_prob, scales = "free", ncol = 1) +
