...
n_subjects_per_group <- 50
eff_probs <- rep(c(0.5, 0.7, 0.8), times = 5) #define the no. of simulations here 
keep_fits <- TRUE
...
```{r}
# Function for effect posterior prediction 
...
effect_from_sim_study <- function(fit) {
  ...
  expected_sum <- function(x) {
    col_values <- as.integer(colnames(x)) #TODO: what does - 1 do here? 
...
```{r}
if(keep_fits) {
  fits_logit <- brm_parallel(args_shared = list(formula = bf(proact_formula, family = cumulative()) + set_rescor(FALSE), adapt_delta = 0.95, init = 0.1, chains = 2, iter = 3000, warmup = 1000), args_per_fit = args_per_fit, cache_dir = cache_dir, cache_fits = TRUE, future.globals = FALSE)
...
fits_lmer_splines <- future.apply::future_lapply(sims, function(data_for_lmer) {
    data_for_lmer$alsfrs_sum_subset <-  rowSums(data_for_lmer[, questions_to_fit]) #here summing all questions to fit (that is what I will manipulate with)
