...
proact_standardized <- proact_full %>%
    ...
    filter_out(is.na(alsfrs_dly_mnths) | alsfrs_dly_mnths <0)

# throw out any time dupes as can cause issues
proact_standardized <- proact_standardized |>
    group_by(subject_id) |>
    mutate(dupe = duplicated(alsfrs_dly_mnths)) |>
    ungroup() |>
    filter_out(dupe == TRUE) |>
    dplyr::select(-dupe)
...
ls_dat <- lapply(1:length(eff_probs), function(m)
    simulate_data_from_registry(proact_standardized |>
                                ...
                                max_measurements_per_subject = 6,
...
# save data
saveRDS(ls_dat, paste0(cache_dir, "/sim_proactdata_4Q.RDS"))
...
# set filename pars
stem1 <- paste0("_PROACT_compare_copula_Q4_n", n_subjects_per_group)
...
brmsnutfits1 <- readRDS(paste0(cache_dir, "/", "2026-09-01_PROACT_compare_copula_Q4_n50_mvprob_400.RDS"))
...
brmsnutfits2 <- readRDS(paste0(cache_dir, "/", "2026-09-01_PROACT_compare_copula_Q4_n50_mvprobhet_400.RDS"))
...
brmsnutfits3 <- readRDS(paste0(cache_dir, "/", "2026-09-01_PROACT_compare_copula_Q4_n50_mvstudt_n3_400.RDS"))
...
brmsnutfits4 <- readRDS(paste0(cache_dir, "/", "2026-09-01_PROACT_compare_copula_Q4_n50_mvstudt_nubeta0.1_400.RDS"))
...
brmsnutfits5 <- readRDS(paste0(cache_dir, "/", "2026-09-01_PROACT_compare_copula_Q4_n50_mvstudt_nubeta0.9_400.RDS"))
...
brmsnutfits6 <- readRDS(paste0(cache_dir, "/", "2026-09-01_PROACT_compare_copula_Q4_n50_mvstudthetsked_nu3_400.RDS"))
...
# combine summaries
all_summs <- rbind(
    summ_mvphet1, summ_mvphet2, summ_mvphet3,
